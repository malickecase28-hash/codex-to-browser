...
export class ConversationRegistry {
  ...
  async get(key: string): Promise<ConversationRecord | undefined> {
    const normalizedKey = normalizeKey(key);
    const path = this.recordPath(normalizedKey);

    try {
      const parsed: unknown = JSON.parse(await readFile(path, "utf8"));
      if (!isConversationRecord(parsed)) throw new Error(`Invalid conversation registry record: ${path}`);
      if (parsed.key !== normalizedKey) throw new Error(`Conversation registry key mismatch: ${path}`);
      return parsed;
    } catch (error) {
  ...
  async find(keyOrAlias: string): Promise<ConversationRecord | undefined> {
  ...
  async remember(args: RememberConversationArgs): Promise<ConversationRecord> {
    ...
    return this.withMutationLock(async () => {
      ...
      if (args.conversationId !== undefined && args.conversationId.trim().length === 0) throw new Error("conversationId must not be empty.");
      if (args.url !== undefined && args.url.trim().length === 0) throw new Error("URL must not be empty.");
  ...
  async touch(key: string): Promise<ConversationRecord | undefined> {
    const existing = await this.get(key);
  ...
  async list(): Promise<ConversationRecord[]> {
    ...
    for (const name of names) {
      ...
      try {
        const parsed: unknown = JSON.parse(await readFile(join(this.stateRoot, name), "utf8"));
        if (!isConversationRecord(parsed)) throw new Error(`Invalid conversation registry record: ${join(this.stateRoot, name)}`);
        records.push(parsed);
      } catch (error) {
  ...
  private async writeRecordFile(record: ConversationRecord, target: string): Promise<void> {
...
export function defaultConversationStateRoot(): string {
  if (platform() === "darwin") return join(homedir(), "Library", "Application Support", "codex-chatgpt-control", "conversations-v1");
  if (platform() === "win32") {
...
function isConversationRecord(value: unknown): value is ConversationRecord {
  ...
  if (record.conversationId !== undefined && (typeof record.conversationId !== "string" || record.conversationId.trim().length === 0)) return false;
  if (record.url !== undefined && (typeof record.url !== "string" || record.url.trim().length === 0)) return false;
