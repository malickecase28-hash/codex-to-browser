import { riskForCommand } from "../safety/risk.js";
const descriptors = [
...
export function helpText(topic) {
    if (topic !== undefined) {
        ...
        if (descriptor === undefined)
            return `No ChatGPT browser-control command is registered as "${topic}".`;
        return [
    ...
    return [
        ...
        "",
        ...["workflow", "diagnostic", "report", "primitive"]
            .flatMap(layer => [
            `${layer}:`,
...
function workflowArgs(name) {
    if (name === "find-open-copy-latest")
        return { query: "history search query" };
    if (name === "find-open-ask-read")
        return { query: "history search query", prompt: "message to send" };
    if (name === "attach-ask-read")
        return { files: "absolute local file paths", prompt: "message to send" };
    if (name === "ask-and-download")
        return { prompt: "message to send", destDir: "download destination directory" };
    if (name === "two-turn")
        return { first: "first message", second: "second message" };
    if (name === "new-ask-read")
        return { prompt: "message to send" };
    if (name === "work.start") {
        return {
...
function workflowDefaults(name) {
    if (name === "copyLatest" || name === "find-open-copy-latest")
        return { prefer: "clipboard", format: "markdown" };
    if (name === "runPlan")
        return {};
    return { wait: true, read: { format: "markdown" } };
}
function diagnosticArgs(name) {
    if (name === "doctor-upload")
        return {};
    return {
...
function reportArgs(name) {
    if (name === "redacted-run-report")
        return { result: "CommandResult to persist" };
...
function primitiveArgs(name) {
    ...
    if (name === "messages.readLatest")
        return { role: "assistant or user", format: "markdown, normalized_text, visible_text, html, blocks, or all" };
    if (name === "messages.status")
        return { maxPreviewChars: "maximum latest-assistant preview characters to return; defaults to 240" };
    if (name === "messages.stop")
        return { confirmStop: "must be true to stop a visible response", timeoutMs: "maximum time to verify generation became inactive" };
    if (name === "experience.detect")
        return { timeoutMs: "optional timeout for attaching to the ChatGPT page" };
    if (name === "experience.open")
        return { experience: "chat or work", timeoutMs: "optional verified-switch timeout" };
    if (name === "configuration.inspect")
        return { experience: "optional expected chat or work surface", includeOptions: "open visible axis menus to enumerate options; defaults to true", timeoutMs: "optional inspection timeout" };
    if (name === "configuration.apply")
        return { experience: "optional target chat or work surface", desired: "model, intelligence, effort, speed, and/or modelVersion values", strict: "fail when visible postconditions are not verified; defaults to true", timeoutMs: "optional selection timeout" };
    if (name === "work.status")
        return { includeArtifacts: "include the latest visible artifact inventory; defaults to false for lightweight polling", maxPreviewChars: "maximum assistant preview characters" };
    if (name === "work.wait")
        return { timeoutMs: "optional Work wait timeout", stableMs: "required response stability window", pollMs: "polling interval" };
    if (name === "work.steer")
        return { prompt: "visible steering message for the current task", wait: "optional wait behavior", read: "optional response capture behavior" };
    if (name === "work.readLatest")
        return { format: "markdown, normalized_text, visible_text, html, blocks, or all", maxChars: "optional capture limit" };
    if (name === "artifacts.listLatest")
        return { kind: "artifact kind; currently image", max: "maximum artifacts to return" };
    if (name === "artifacts.wait")
        return { kind: "artifact kind; currently image", afterArtifactCount: "baseline artifact count", requireDownload: "wait until a download affordance is visible" };
    if (name === "artifacts.downloadLatest")
        return { destDir: "download destination directory", prefer: "download_control or visible_image_source" };
    if (name === "files.downloadLatest")
        return { destDir: "download destination directory", filenamePattern: "optional case-insensitive regular expression for the expected filename", from: "latest_assistant, visible_conversation, or assistantIndex" };
    if (name === "response.copy")
        return { prefer: "clipboard or dom", format: "markdown, normalized_text, visible_text, html, blocks, or all" };
    if (name.startsWith("threads.search"))
        return { query: "history search query" };
    ...
    if (name.startsWith("files.attach"))
        return { paths: "absolute local file paths" };
    if (name === "projects.sources.list")
        return { projectUrl: "ChatGPT Project URL such as https://chatgpt.com/g/g-p-.../project", existingTab: "optional exact existing-tab policy", timeoutMs: "optional browser timeout" };
    if (name === "projects.sources.planAdd")
        return { projectUrl: "ChatGPT Project URL", files: "explicit absolute local file paths", batchSize: "optional upload batch size" };
    if (name === "projects.sources.add")
        return { projectUrl: "ChatGPT Project URL", files: "explicit absolute local file paths", confirmMutation: "must be true to mutate Project Sources", batchSize: "optional upload batch size" };
    if (name === "modes.set") {
        return {
...
function primitiveExamples(name) {
    ...
    if (name === "experience.detect")
        return [`await chatgpt.experience.detect();`];
    if (name === "experience.open")
        return [`await chatgpt.experience.open({ experience: "work" });`];
    if (name === "configuration.inspect")
        return [`await chatgpt.configuration.inspect({ experience: "work" });`];
    if (name === "configuration.apply")
        return [`await chatgpt.configuration.apply({ experience: "work", desired: { model: "GPT-5.6 Sol", effort: "High", speed: "Fast" }, strict: true });`];
    if (name === "work.status")
        return [`await chatgpt.work.status();`];
    if (name === "work.wait")
        return [`await chatgpt.work.wait({ timeoutMs: 600000 });`];
    if (name === "work.steer")
        return [`await chatgpt.work.steer({ prompt: "Focus the comparison on deployment ergonomics." });`];
    if (name === "work.readLatest")
        return [`await chatgpt.work.readLatest({ format: "markdown" });`];
    if (name === "files.preflight") {
        return [
...
function primitiveBlockers(name) {
    if (name === "files.preflight")
        return ["not_found", "permission", "upload_failed"];
    if (name.startsWith("files.attach"))
        return ["browser_bridge_unavailable", "login_required", "permission", "upload_failed"];
    if (name.startsWith("files.download"))
        return ["browser_bridge_unavailable", "login_required", "download_unavailable"];
    if (name === "projects.sources.planAdd")
        return ["not_found", "permission", "upload_failed"];
    if (name.startsWith("projects.sources."))
        return ["browser_bridge_unavailable", "login_required", "selector_drift", "confirmation", "permission", "upload_failed"];
    if (name.startsWith("artifacts."))
        return ["browser_bridge_unavailable", "login_required", "artifact_unavailable", "artifact_selector_drift", "artifact_download_unavailable"];
    if (name.startsWith("modes.") || name.startsWith("tools.") || name.startsWith("experience.") || name.startsWith("configuration.") || name.startsWith("work."))
        return ["browser_bridge_unavailable", "login_required", "selector_drift"];
