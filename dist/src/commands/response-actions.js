...
import { readLatestMessage, readMessages } from "../dom/messages.js";
...
export async function copyResponse(env, args = {}) {
    ...
    try {
        ...
        if (args.prefer !== "dom") {
            ...
            if (copied !== undefined) {
                ...
                return withCommandOutputText(resultOk(data, await contextFromPage(page)));
            }
        }
        ...
        if (latest !== undefined) {
            ...
            const data = copiedResponseFromExtracted(latest, "dom", fallbackReason);
            return withCommandOutputText(resultOk(data, await contextFromPage(page), data.warnings ?? [fallbackReason]));
        }
        return {
            ok: false,
            status: "not_found",
...
function mergeResponseMetadata(data, latest) {
    if (latest === undefined)
        return;
    if (latest.markdown !== undefined && data.markdown === undefined)
