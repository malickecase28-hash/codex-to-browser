...
import { askMessage, composeMessage, messageStatus, readLatest, submitMessage, waitForMessage } from "./messages.js";
import { ensurePage } from "./session.js";
const NEW_WORK_LABELS = localeLabels.newWork;
export async function startWork(env, args) {
    ...
    try {
        ...
        if (args.wait === true || typeof args.wait === "object") {
            ...
            if (baselineTurnCount !== undefined)
                waitArgs.afterTurnCount = baselineTurnCount;
            if (baselineAssistantTurnCount !== undefined)
                waitArgs.afterAssistantTurnCount = baselineAssistantTurnCount;
            waitResult = await waitForMessage(env, waitArgs);
            if (waitResult.data !== undefined)
                data.wait = waitResult.data;
        ...
        if (waitResult !== undefined && !waitResult.ok) {
            ...
            if (outputText !== undefined)
                partial.output_text = outputText;
            return partial;
        }
        return resultOk(data, await workContext(env, surface.data?.selectorProfile), ["Work task submission uses matching-turn recovery and will not blindly resubmit the prompt."]);
    }
    catch (error) {
        return resultError(error instanceof Error ? error : new Error(String(error)), await workContext(env));
    }
}
export async function workStatus(env, args = {}) {
    const ready = await requireWork(env);
    if (!ready.ok)
        return forwardCommandFailure(ready);
...
export async function waitForWork(env, args = {}) {
    const ready = await requireWork(env);
    if (!ready.ok)
        return forwardCommandFailure(ready);
    return markWorkResult(await waitForMessage(env, args), ready.data?.selectorProfile);
}
export async function steerWork(env, args) {
...
function transactionalWorkRoutingBlocker(operationId, action) {
...
export async function readLatestWork(env, args = {}) {
    const ready = await requireWork(env);
    if (!ready.ok)
        return forwardCommandFailure(ready);
...
async function workTaskRef(env, baselineTurnCount, baselineAssistantTurnCount) {
    ...
    if (context.url !== undefined)
        task.url = context.url;
    if (context.conversationId !== undefined)
        task.conversationId = context.conversationId;
    if (context.title !== undefined)
        task.title = context.title;
    if (baselineTurnCount !== undefined)
        task.baselineTurnCount = baselineTurnCount;
    if (baselineAssistantTurnCount !== undefined)
        task.baselineAssistantTurnCount = baselineAssistantTurnCount;
    return task;
}
