import { enumerateVisibleMenuItems } from "../dom/menus.js";
...
const WORK_AXES = ["model", "effort", "speed"];
...
const CONFIGURATION_AXIS_ORDER = [
    "model",
    "intelligence",
    "effort",
    "speed",
...
export async function inspectConfiguration(env, args = {}) {
    ...
    const page = env.page;
    try {
        const detected = await detectExperience(env, args.timeoutMs === undefined ? {} : { timeoutMs: args.timeoutMs });
        ...
        const rootOpened = experience !== "unknown" && await waitForConfigurationRoot(page, experience, args.timeoutMs);
        if (rootOpened) {
        ...
        const data = configurationInspectionFromSurface(experience, detected.data.selectorProfile, detected.data.evidence, panel, rootItems);
        if (args.includeOptions !== false && experience === "work" && panel.axisRows.length > 0) {
            for (const axis of WORK_AXES) {
                if (!data.availableAxes.includes(axis))
                    continue;
...
async function waitForConfigurationRoot(page, experience, timeoutMs) {
    const discoveryMs = Math.min(timeoutMs ?? CONFIGURATION_CONTROL_DISCOVERY_TIMEOUT_MS, CONFIGURATION_CONTROL_DISCOVERY_TIMEOUT_MS);
    const attempts = Math.max(1, Math.ceil(Math.max(0, discoveryMs) / CONFIGURATION_CONTROL_POLL_MS));
    for (let attempt = 0; attempt < attempts; attempt += 1) {
        if (await openConfigurationRoot(page, experience))
            return true;
...
export async function applyConfiguration(env, args) {
    const boot = await ensurePage(env);
    if (!boot.ok) {
        return boot;
    }
    ...
    try {
        ...
        for (const [axis, requested] of selectionEntries(desired)) {
            ...
            if (selection === undefined) {
                return configurationFailure(page, before, desired, selected, `Configuration option "${requested}" for ${axis} was not found or was ambiguous on the ${before.experience} surface.`, "configuration_option_not_found", before.options[axis]?.map(option => option.label));
            }
...
export function configurationInspectionFromSurface(experience, detectedProfile, evidence, panel, menuItems) {
    const active = {};
    const options = {};
    const availableAxes = [];
    ...
    if (experience === "work") {
        for (const row of panel.axisRows) {
            if (!availableAxes.includes(row.axis))
                availableAxes.push(row.axis);
            if (row.value !== undefined && row.value.length > 0)
                active[row.axis] = row.value;
        }
...
async function selectWorkAxis(env, axis, requested, timeoutMs) {
    ...
    const retryWindowMs = Math.min(timeoutMs ?? CONFIGURATION_CONTROL_DISCOVERY_TIMEOUT_MS, CONFIGURATION_CONTROL_DISCOVERY_TIMEOUT_MS);
    const attempts = Math.max(2, Math.min(CONFIGURATION_SELECTION_MAX_ATTEMPTS, Math.ceil(Math.max(0, retryWindowMs) / CONFIGURATION_SELECTION_RETRY_MS) + 1));
    for (let attempt = 0; attempt < attempts; attempt += 1) {
...
async function openWorkAxisOptions(env, axis, allowRootRetry = true) {
    const page = env.page;
    if (!await ensureWorkAdvancedPanel(page))
        return [];
    ...
    if (point !== undefined && await movePointerWithCdp(env, point)) {
        ...
        if (retriedOptions.length > 0)
            return retriedOptions;
    }
    ...
    if (clickedOptions.length > 0 || !allowRootRetry)
...
function filterWorkAxisOptions(items, axis) {
    return items.filter(item => {
        if (isConfigurationAxisRow(item.label))
            return false;
...
async function openConfigurationRoot(page, experience) {
    ...
    if (typeof page.evaluate === "function") {
        const clicked = await page.evaluate((surface) => {
            ...
            const roots = Array.from(new Set(composerRoots.length > 0
                ? composerRoots
                : (main === null ? [] : [main])));
            const controls = Array.from(new Set(roots.flatMap(root => Array.from(root.querySelectorAll("button, [role='button']")))))
                .filter(visible);
            const matches = controls.filter(control => {
                const html = control;
                const label = normalize(control.getAttribute("aria-label") ?? html.innerText ?? control.textContent ?? "");
                const testId = control.getAttribute("data-testid") ?? "";
                if (/send|voice|microphone|attach|upload|add files|plus/i.test(`${label} ${testId}`))
                    return false;
                if (/model-switcher|model-selector|mode-selector/i.test(testId))
                    return true;
                return surface === "work"
...
async function ensureWorkAdvancedPanel(page) {
    ...
    if (panel.axisRows.length > 0)
        return true;
    if (!await openConfigurationRoot(page, "work"))
        return false;
    const reopenedPanel = await readConfigurationPanel(page);
    if (reopenedPanel.axisRows.length > 0)
        return true;
...
async function readConfigurationPanel(page) {
    ...
    return page.evaluate((axisLabels) => {
        const normalize = (value) => value.replace(/\s+/g, " ").trim();
        const normalizedAxes = Object.fromEntries(Object.entries(axisLabels).map(([axis, labels]) => [
            axis,
        ...
        const overlays = Array.from(document.querySelectorAll("[role='menu'], [role='listbox'], [data-radix-popper-content-wrapper], [data-radix-menu-content]")).filter(visible);
        const roots = overlays.length > 0 ? overlays : Array.from(document.querySelectorAll("main")).filter(visible);
        const rows = roots.flatMap(root => Array.from(root.querySelectorAll("button, [role='button'], [role='menuitem'], [role='menuitemradio'], [role='option']"))).filter(visible);
        const axisRows = [];
        ...
        const composerRoots = formRoots.length > 0
            ...
            : testIdRoots.length > 0
                ...
                : classRoots;
        const main = document.querySelector("main");
        const openerRoots = Array.from(new Set(composerRoots.length > 0
            ? composerRoots
            : (main === null ? [] : [main])));
        const openerCandidates = Array.from(new Set(openerRoots.flatMap(root => Array.from(root.querySelectorAll("button, [role='button']")))))
            .filter(visible)
            .map(control => {
            const html = control;
            return {
...
async function clickVisibleMenuItem(page, item) {
    ...
    return clickIfUnique(page.locator?.("button, [role='button'], [role='menuitem'], [role='menuitemradio'], [role='option']")?.filter?.({ hasText: exactText }));
}
function findConfigurationOption(items, requested) {
    ...
    for (const wanted of semanticLabels) {
        const matches = items.filter(item => normalizeForLabelMatch(item.label) === normalizeForLabelMatch(wanted)
...
export function configurationMatchesSelection(inspection, desired) {
...
function menuItemToOption(item) {
    ...
    if (item.hasPopup !== undefined)
        option.hasSubmenu = item.hasPopup;
    return option;
}
...
async function clickIfUnique(locator) {
    ...
    if (visible.length !== 1 || visible[0]?.click === undefined)
        return false;
...
function forwardFailure(result) {
    ...
    if (result.output_text !== undefined)
        forwarded.output_text = result.output_text;
    if (result.reportPath !== undefined)
        forwarded.reportPath = result.reportPath;
