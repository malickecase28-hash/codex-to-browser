...
import { commandDescriptors } from "../commands/registry.js";
...
export function validateSurfaceDrift(packageRootInput = defaultPackageRoot()) {
...
export function collectSurfaceDriftModel(packageRootInput = defaultPackageRoot()) {
    const packageRoot = toPath(packageRootInput);
    const contractRoot = join(packageRoot, "contracts", "v1");
    const policy = readJson(join(contractRoot, "surface-drift-policy.json"));
    const repoRoot = resolve(packageRoot, "..", "..");
    const pythonRoot = resolvePythonRoot(packageRoot);
    const blockerKinds = readBlockerKinds(join(packageRoot, "src", "types.ts"));
    const descriptorFixture = readJson(join(contractRoot, "fixtures", "command-descriptors.json"));
    const capabilitiesFixture = readJson(join(contractRoot, "fixtures", "backend-capabilities.json"));
    const blockerProfilesFixture = readJson(join(contractRoot, "fixtures", "blocker-explanation-profiles.json"));
    const paritySuite = readJson(join(contractRoot, "parity-suite.json"));
    ...
    return {
        ...
        blockerExplanations: blockerKinds.map(kind => {
            const explanation = explainCommandBlocker({ kind: kind, message: `Synthetic ${kind} blocker for surface drift validation.` });
            return {
                kind,
...
export function validateSurfaceDriftModel(model) {
    const errors = [];
    ...
    compareSets("blocker explanation profile fixture kinds", model.blockerExplanationFixtureKinds, model.blockerKinds, errors);
...
export function generatedBlockerCoverageSection(explanations) {
    return [
...
function collectDocs(packageRoot, policy, pythonRoot) {
    const paths = new Set([
...
function validatePolicy(model, errors) {
    ...
    validateUniqueExemptions("descriptorBackendCommandExemptions", policy.descriptorBackendCommandExemptions.map(exemption => exemption.descriptor), errors);
    ...
    for (const anchor of policy.docAnchors) {
        if (anchor.paths.length === 0)
            errors.push(`doc anchor ${anchor.id} must include at least one path`);
        if (anchor.terms.length === 0)
            errors.push(`doc anchor ${anchor.id} must include at least one term`);
    }
}
function validateDatedExemption(label, exemption, errors) {
...
function validateUniqueExemptions(label, values, errors) {
    const seen = new Set();
    for (const value of values) {
        if (seen.has(value))
            errors.push(`${label} contains duplicate entry ${value}`);
        seen.add(value);
    }
}
...
function normalizePolicyForLayout(policy, repoRoot) {
...
function readBlockerKinds(typesPath) {
    ...
    return [...match[1].matchAll(/"([^"]+)"/g)].map(item => {
        const kind = item[1];
        if (kind === undefined)
            throw new Error("Unable to parse BlockerKind literal.");
        return kind;
    });
}
function readBackendDispatchCommands(sessionPath) {
    ...
    return sortedUnique([...match[1].matchAll(/case "([^"]+)":/g)].map(item => {
        ...
        if (command === undefined)
            throw new Error("Unable to parse backend dispatch command.");
        return command;
    }));
}
function readPythonCommands(pythonRoot, knownBackendCommands) {
    const known = new Set(knownBackendCommands);
...
function resolvePythonRoot(packageRoot) {
    const repoRoot = resolve(packageRoot, "..", "..");
    const candidates = [
