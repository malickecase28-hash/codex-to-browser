...
import { createChatGPT as createBaseChatGPT } from "../client.js";
...
import { createDevAutonomousApi } from "./autonomous-api.js";
import { bindCodexLocalPlanningIdentity, codexLocalIdentityOptions } from "./autonomous-local-identity.js";
import { createCodexCliAutonomousLocalPort } from "./codex-cli-safe-local-port.js";
import { ChatGPTAutonomousPort } from "./autonomous-chatgpt-port.js";
...
function devRuntimeEnv(options) {
    ...
    if (options.compatibility !== undefined)
        env.compatibility = options.compatibility;
...
async function requireOwnedDevRuntime(env) {
    if (env.page !== undefined) {
        ...
        if (authoritativeTabId === undefined) {
            throw new DevOrchestratorError("tab_ownership_unavailable", "Development orchestration requires an authoritative browser-bound tab identity; PageLike.id and PageLike.tabId are not ownership evidence.");
        }
        if (env.expectedTabId !== undefined && env.expectedTabId !== authoritativeTabId) {
            throw new DevOrchestratorError("route_drift", "The development runtime no longer owns the expected physical ChatGPT tab.");
        }
    ...
    if (attached.tabId === undefined) {
        throw new DevOrchestratorError("tab_ownership_unavailable", "The connected browser created a ChatGPT tab without an authoritative provider tab identity.");
    }
...
export function createChatGPT(options = {}) {
    ...
    const ui = createDevOrchestrator(Object.freeze({
    ...
    const autonomousRoot = resolve(autonomousOptions?.stateRoot
    ...
    if (local === undefined && autonomousOptions?.localCodex !== undefined) {
        ...
        local = bindCodexLocalPlanningIdentity(codexLocal, codexLocalIdentityOptions(codexOptions, join(autonomousRoot, "execution-identities")));
    }
