import { DevAutonomousEngine, DevAutonomousPortError } from "./autonomous-engine.js";
import { DevAutonomousStoreError } from "./autonomous-store.js";
import { FileDevAutonomousPlanningSpecStore } from "./autonomous-planning-store.js";
const DEFAULT_MAX_STEPS = 128;
const MAX_STEPS = 10_000;
export function createDevAutonomousApi(options) {
    const local = options.local ?? unavailableLocalPort();
    const planningVerifier = options.local?.verifyPlanningSpec;
    const engine = new DevAutonomousEngine(options.store, options.chat, local, options.maxParallelTasks === undefined ? {} : { maxParallelTasks: options.maxParallelTasks });
    ...
    const requirePlanner = () => {
        if (planner === undefined) {
            throw new DevAutonomousPortError("planner_unavailable", true, "Autonomous master planning requires a visible-ChatGPT planner port.");
        }
        return planner;
    };
    const verifyExecutionIdentity = async (spec) => {
        if (planningVerifier !== undefined)
            await planningVerifier.call(options.local, spec);
    };
        ...
        bootstrap: async (spec, planningOptions) => {
            try {
                ...
                if (identity === undefined) {
                    throw new DevAutonomousPortError("workflow_identity_mismatch", false, "The existing workflow has no immutable master-planning identity. Use a new workflow ID instead of retroactively binding an objective.");
                }
                await planningStore.claim(spec);
                await verifyExecutionIdentity(spec);
                if (existing.projectKey !== spec.projectKey
                    || existing.plannerConversationKey !== spec.plannerConversationKey) {
                    throw new DevAutonomousPortError("workflow_identity_mismatch", false, "An existing autonomous workflow ID belongs to a different Project or planner conversation.");
                }
                return existing;
            }
            catch (error) {
                if (!(error instanceof DevAutonomousStoreError) || error.code !== "workflow_not_found")
                    throw error;
            }
            const plannerPort = requirePlanner();
            await planningStore.claim(spec);
...
function unavailableLocalPort() {
    const blocked = async () => {
        throw new DevAutonomousPortError("local_executor_unavailable", true, "Autonomous repository work requires an injected local executor with implementation, independent test, push, and integration capabilities.");
    };
    return Object.freeze({
        implement: blocked,
        test: blocked,
