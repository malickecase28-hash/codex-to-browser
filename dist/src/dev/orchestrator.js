...
function errorResult(error, now) {
    ...
    const status = devError.code === "not_found"
        ? "not_found"
...
function requireMutationConfirmation(confirmed, label) {
    if (confirmed !== true) {
        throw new DevOrchestratorError("confirmation_required", `Explicit caller confirmation is required before ${label}.`);
    }
}
function exactlyOne(items, predicate, label) {
    const matches = items.filter(predicate);
    if (matches.length === 0)
        throw new DevOrchestratorError("not_found", `${label} was not found.`);
    if (matches.length > 1)
        throw new DevOrchestratorError("ambiguous_match", `${label} matched more than one visible target.`);
    return matches[0];
}
...
function projectMatches(record, ref) {
    ...
    if (ref.projectId !== undefined && record.projectId !== ref.projectId)
        return false;
    if (ref.url !== undefined && record.url !== ref.url)
        return false;
    if (ref.name !== undefined && normalizedName(record.name) !== normalizedName(ref.name))
        return false;
...
function plannerMatches(record, ref) {
    ...
    if (ref.taskId !== undefined && record.taskId !== ref.taskId)
        return false;
    if (ref.name !== undefined && normalizedName(record.name) !== normalizedName(ref.name))
        return false;
...
function workerMatches(record, ref) {
    if (typeof ref === "string")
        return record.workerId === ref || normalizedName(record.name) === normalizedName(ref);
    if (ref.workerId !== undefined && record.workerId !== ref.workerId)
        return false;
    if (ref.name !== undefined && normalizedName(record.name) !== normalizedName(ref.name))
        return false;
...
function assertProjectSpec(spec) {
    ...
    if (spec.name.length > 200)
        throw new DevOrchestratorError("invalid_spec", "Project name is too long.", false);
}
...
function projectDesired(project, spec) {
    if (normalizedName(project.name) !== normalizedName(spec.name))
        return false;
    if (spec.description !== undefined && project.description !== spec.description)
        return false;
    if (spec.instructions !== undefined && project.instructions !== spec.instructions)
        return false;
    if (spec.defaultModel !== undefined && project.defaultModel !== spec.defaultModel)
        return false;
    return true;
}
function plannerDesired(task, spec) {
    if (normalizedName(task.name) !== normalizedName(spec.name))
        return false;
    if (task.prompt !== undefined && task.prompt !== spec.prompt)
        return false;
    if (task.schedule !== undefined && task.schedule !== spec.schedule)
        return false;
    if (spec.timezone !== undefined && task.timezone !== undefined && task.timezone !== spec.timezone)
        return false;
    if (spec.enabled !== undefined && task.enabled !== spec.enabled)
        return false;
    if (spec.model !== undefined && task.model !== undefined && task.model !== spec.model)
        return false;
    return true;
}
...
export function createDevOrchestrator(runtime, options = {}) {
    ...
    const createProjectMutation = async (spec) => {
        ...
        if (prior !== undefined)
            return receiptProject(prior, { name: spec.name });
        ...
        try {
            ...
            if (error instanceof DevOrchestratorError && error.code !== "mutation_uncertain")
                throw error;
            const after = await projectSnapshot(runtime, adapter, store);
    ...
    const updateProjectMutation = async (ref, changes) => {
        ...
        const prior = await store.receipt(key);
        if (prior !== undefined)
            return receiptProject(prior, { projectId: project.projectId });
        try {
            ...
            for (const [field, value] of Object.entries(changes)) {
                if (field === "idempotencyKey" || value === undefined)
                    continue;
                const observed = updated[field] ?? visible[field];
                if (observed !== value)
                    throw new DevOrchestratorError("mutation_uncertain", "Project update postconditions were not verified.");
            }
    ...
    const deleteProjectMutation = async (ref, options) => {
        ...
        const prior = await store.receipt(key);
        if (prior !== undefined)
            throw new DevOrchestratorError("not_found", "This Project deletion was already committed; the destructive mutation will not be repeated.", false);
        const before = await projectSnapshot(runtime, adapter, store);
        const project = exactlyOne(before, item => projectMatches(item, ref), "Project");
        try {
    ...
    const createPlannerMutation = async (spec) => {
        ...
        const prior = await store.receipt(key);
        if (prior !== undefined)
            return receiptPlanner(prior, { name: spec.name });
    ...
    const updatePlannerMutation = async (ref, changes) => {
        ...
        const prior = await store.receipt(key);
        if (prior !== undefined)
            return receiptPlanner(prior, { taskId: task.taskId });
        try {
            ...
            for (const [field, value] of Object.entries(changes)) {
                ...
                const observed = updated[field] ?? visible[field];
                if (observed !== value)
                    throw new DevOrchestratorError("mutation_uncertain", "Planner update postconditions were not verified.");
            }
    ...
    const deletePlannerMutation = async (ref, options) => {
        ...
        const prior = await store.receipt(key);
        if (prior !== undefined)
            throw new DevOrchestratorError("not_found", "This Planner deletion was already committed; the destructive mutation will not be repeated.", false);
        const before = await plannerSnapshot(runtime, adapter, store);
        const task = exactlyOne(before, item => plannerMatches(item, ref), "Planner task");
        try {
    ...
    const runNowMutation = async (ref, explicitKey) => {
        if (adapter.runPlannerTaskNow === undefined)
            throw new DevOrchestratorError("ui_unsupported", "The live Planner UI does not expose Run now.");
        ...
        if (prior !== undefined) {
            ...
            if (created.length !== 1)
                throw new DevOrchestratorError("mutation_uncertain", "A prior Run now receipt exists but its exact run cannot be reconciled.");
    ...
    const scheduleWorker = (record) => {
        if (record.status !== "running" || record.runPolicy?.enabled === false)
            return;
    ...
    const projects = Object.freeze({
        ...
        open: (ref) => safe(now, async () => {
            ...
            if (matches.length > 1)
                throw new DevOrchestratorError("ambiguous_match", "Project ensure found duplicate exact Project names.");
            if (matches.length === 0)
                return createProjectMutation(spec);
        ...
        delete: (ref, options) => safe(now, () => deleteProjectMutation(ref, options)),
        chats: Object.freeze({
    ...
    const planner = Object.freeze({
        ...
        delete: (ref, options) => safe(now, () => deletePlannerMutation(ref, options)),
        setEnabled: (ref, enabled, options) => safe(now, () => setEnabledMutation(ref, enabled, options?.idempotencyKey)),
        runs: (ref) => safe(now, async () => {
            const task = await resolvePlanner(ref);
            return runtime.run(env => adapter.listPlannerRuns(env, task));
        }),
        runNow: (ref, options) => safe(now, () => runNowMutation(ref, options?.idempotencyKey))
    });
    const worker = Object.freeze({
        start: (spec) => safe(now, async () => {
            if (spec.name.trim().length === 0)
                throw new DevOrchestratorError("invalid_spec", "Worker name is required.", false);
            const workers = await store.workers();
...
export function runtimeFromEnvironment(env) {
