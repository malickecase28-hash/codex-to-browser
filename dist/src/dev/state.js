...
export function devDigest(value) {
...
async function withStateQueue(key, callback) {
    ...
    try {
        ...
        release();
        if (stateQueues.get(key) === chained)
...
function validateDocument(value, label) {
    ...
    const record = value;
    if (record.schemaVersion !== DEV_STATE_SCHEMA_VERSION
...
export class DevStateStore {
    ...
    async loadDocument(name) {
        ...
        return validateDocument(await readJson(this.path(name), emptyDocument(now)), name);
    }
