...
import { OPERATION_REQUEST_SCHEMA_VERSION } from "../operations/types.js";
import { DevAutonomousPortError, deterministicDevOperationId, deterministicDevWatcherId } from "./autonomous-engine.js";
import { devAutonomousPlannerPrompt, devAutonomousPlanningDigest, parseDevAutonomousPlannerResponse, validateDevAutonomousPlanningSpec } from "./autonomous-planner.js";
...
export class ChatGPTAutonomousPort {
    chatgpt;
    conversations;
    watcherStore;
    watchers;
    ...
    async planWorkflow(spec, options = {}) {
        ...
        if (response === undefined) {
            throw new DevAutonomousPortError("planner_response_pending", true, "The master planner response is still pending; retrying will resume the same durable planner turn.");
        }
    ...
    async beginGuidance(input) {
        if (input.task.workerReview?.status === "revision_required" && input.workerReviewGuidance === undefined) {
            throw new DevAutonomousPortError("review_guidance_unavailable", true, "The exact worker review guidance must be rehydrated before a revision turn can be submitted.");
        }
        if (input.workerReviewGuidance !== undefined)
            validateRevisionGuidance(input.workerReviewGuidance);
        const conversation = await this.resolveGuidanceConversation(input.workflow, input.conversationKey, input.task);
        await this.beginTurn({
            ...
            prompt: guidancePrompt(input.workflow, input.task, input.localTestFailure, input.workerReviewGuidance)
        });
    ...
    async collectGuidance(dispatch, options) {
        ...
        return response === undefined
            ...
            : Object.freeze({ status: "completed", responseDigest: response.digest });
    }
    async readGuidance(evidence) {
        const response = await this.turns.readResponse(evidence.watcherId, evidence.responseDigest);
        if (response === undefined) {
            throw new DevAutonomousPortError("guidance_cache_unavailable", true, "The exact worker guidance is not available in the restart-safe turn cache.");
        }
    ...
    async readReviewGuidance(input) {
        ...
        if (turn.kind !== input.kind || turn.logicalConversationKey !== input.conversationKey) {
            throw new DevAutonomousPortError("review_guidance_identity_mismatch", false, "Durable review guidance belongs to a different review kind or logical conversation.");
        }
        const response = await this.turns.readResponse(input.watcherId, input.reviewDigest);
        if (response === undefined) {
            throw new DevAutonomousPortError("review_guidance_unavailable", true, "The exact revision review is unavailable from the restart-safe turn cache.");
        }
        const parsed = parseReviewResult(response.text);
        if (parsed.verdict !== "revision_required") {
            throw new DevAutonomousPortError("review_guidance_mismatch", false, "Durable review evidence does not contain revision guidance.");
        }
        return parsed.guidance;
    }
    async reviewCommit(input) {
        const conversation = await this.requireExistingConversation(input.conversationKey, "The worker conversation that produced implementation guidance is unavailable for commit review.");
        await this.beginTurn({
            workflow: input.workflow,
            conversation,
            logicalConversationKey: input.conversationKey,
    ...
    async reviewIntegration(input) {
        ...
        const conversation = await this.requireExistingConversation(key, "The master planner conversation is unavailable for final integration review.");
        await this.beginTurn({
            workflow: input.workflow,
            conversation,
    ...
    async existingConversation(key) {
        ...
        if (affinity === undefined) {
            throw new DevAutonomousPortError("conversation_affinity_unavailable", true, "The semantic ChatGPT conversation has no exact physical-tab affinity.");
        }
        if (existing.conversationId !== undefined
            && affinity.conversationId !== undefined
            && existing.conversationId !== affinity.conversationId) {
            throw new DevAutonomousPortError("conversation_identity_mismatch", false, "Semantic conversation identity does not match its physical-tab affinity.");
        }
        return existing;
    }
    ...
    async resolvePlannerConversation(workflow, key) {
        const existing = await this.existingConversation(key);
        if (existing !== undefined)
            return existing;
    ...
    async resolveGuidanceConversation(workflow, key, task) {
        const existing = await this.existingConversation(key);
        if (existing !== undefined)
            return existing;
        if (this.provisioner === undefined)
    ...
    async beginTurn(input) {
        ...
        if (existingTurn !== undefined) {
            if (existingTurn.logicalConversationKey !== input.logicalConversationKey
        ...
        const target = creatingConversation
            ? { type: "new", url: projectStartUrl(input.workflow.projectKey) }
        ...
        await this.bindConversationFromOperation(input.logicalConversationKey, inspected.state, creatingConversation);
    ...
    async collectTurn(watcherId, options) {
        ...
        if (text === undefined) {
            throw new DevAutonomousPortError("raw_response_unavailable", true, "The exact autonomous ChatGPT response is no longer available from the operation collector.");
        }
...
export function createChatGPTAutonomousPort(chatgpt, options = {}) {
...
function projectStartUrl(projectKey) {
    ...
    try {
        ...
        throw new DevAutonomousPortError("project_identity_unavailable", false, "Autonomous first-send chat creation requires an exact ChatGPT Project ID or Project URL.");
    }
...
function validateConversationIdentity(identity) {
    if (typeof identity.conversationId !== "string"
...
function guidancePrompt(workflow, task, localTestFailure, workerReviewGuidance) {
    if (localTestFailure !== undefined)
        validateLocalTestFailure(localTestFailure);
    if (workerReviewGuidance !== undefined)
        validateRevisionGuidance(workerReviewGuidance);
...
function validateLocalTestFailure(value) {
    if (!/^sha256:[0-9a-f]{64}$/u.test(value.candidateDigest)
        || !/^sha256:[0-9a-f]{64}$/u.test(value.reportDigest)
        || typeof value.summary !== "string"
        || value.summary.trim().length === 0
        || value.summary.length > 32_768
        || /[\u0000\u000b\u000c\u007f]/u.test(value.summary)) {
        throw new DevAutonomousPortError("task_test_feedback_invalid", false, "Local task-test feedback did not match its bounded digest-bound contract.");
    }
}
function validateRevisionGuidance(value) {
    if (typeof value !== "string"
        ...
        || /[\u0000\u000b\u000c\u007f]/u.test(value)) {
        throw new DevAutonomousPortError("review_guidance_invalid", false, "Worker revision guidance exceeded the bounded durable review contract.");
    }
}
...
function plannerReviewPrompt(workflow, commitSha) {
    return [
...
export function parseReviewResult(text) {
    ...
    for (const candidate of candidates) {
        try {
            ...
            if (record.verdict === "revision_required"
                && keys.length === 2
                && keys[0] === "guidance"
                && keys[1] === "verdict"
    ...
    throw new DevAutonomousPortError("review_response_invalid", true, "The ChatGPT review response did not contain the required strict accepted or revision-guidance object.");
}
