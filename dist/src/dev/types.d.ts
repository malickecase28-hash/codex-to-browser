...
export declare const DEV_STATE_SCHEMA_VERSION: "chatgpt.browser_control.dev_state.v1";
export declare const DEV_RECEIPT_SCHEMA_VERSION: "chatgpt.browser_control.dev_receipt.v1";
export type DevProjectRef = string | Readonly<{
    projectId?: string;
    name?: string;
    url?: string;
}>;
...
export type DevPlannerTaskRef = string | Readonly<{
    taskId?: string;
    name?: string;
}>;
...
export type DevWorkerRef = string | Readonly<{
    workerId?: string;
    name?: string;
}>;
export type DevCheckpointPolicy = Readonly<{
    intervalMs?: number;
    keep?: number;
}>;
export type DevWorkerRunPolicy = Readonly<{
...
export type DevReceiptKind = "project_inspect" | "project_mutation" | "project_reconcile" | "planner_inspect" | "planner_mutation" | "planner_reconcile" | "worker_transition" | "worker_checkpoint";
...
export type DevMutationResult<T> = Readonly<{
    value: T;
    receipt: DevReceipt;
}>;
...
export type DevVisibleBrowserAdapter = Readonly<{
    ...
    listProjectChats(env: RuntimeEnv, project: DevProjectRecord): Promise<readonly Readonly<{
        chatId: string;
        title: string;
        url: string;
    }>[]>;
    openProjectChat(env: RuntimeEnv, project: DevProjectRecord, chat: Readonly<{
        chatId: string;
        title: string;
        url: string;
    }>): Promise<Readonly<{
    ...
    inspectPlanner(env: RuntimeEnv): Promise<Readonly<{
        supported: boolean;
        url?: string;
        observedAt: string;
    }>>;
...
export type DevProjectsApi = Readonly<{
    list(filters?: Readonly<{
        name?: string;
    }>): Promise<CommandResult<readonly DevProjectRecord[]>>;
    ...
    delete(ref: DevProjectRef, options?: Readonly<{
        idempotencyKey?: string;
        confirmMutation?: boolean;
    }>): Promise<CommandResult<DevMutationResult<DevProjectRecord>>>;
    chats: Readonly<{
        list(ref: DevProjectRef): Promise<CommandResult<readonly Readonly<{
            chatId: string;
            title: string;
            url: string;
        }>[]>>;
        open(ref: DevProjectRef, chatRef: string): Promise<CommandResult<Readonly<{
            chatId: string;
            title: string;
            url: string;
        }>>>;
    ...
    context: Readonly<{
        inspect(ref: DevProjectRef): Promise<CommandResult<DevProjectContext>>;
    }>;
}>;
export type DevPlannerApi = Readonly<{
    inspect(): Promise<CommandResult<Readonly<{
        supported: boolean;
        url?: string;
        observedAt: string;
    }>>>;
    ...
    delete(ref: DevPlannerTaskRef, options?: Readonly<{
        idempotencyKey?: string;
        confirmMutation?: boolean;
    }>): Promise<CommandResult<DevMutationResult<DevPlannerTaskRecord>>>;
    setEnabled(ref: DevPlannerTaskRef, enabled: boolean, options?: Readonly<{
        idempotencyKey?: string;
    }>): Promise<CommandResult<DevMutationResult<DevPlannerTaskRecord>>>;
    runs(ref: DevPlannerTaskRef): Promise<CommandResult<readonly DevPlannerRunRecord[]>>;
    runNow(ref: DevPlannerTaskRef, options?: Readonly<{
        idempotencyKey?: string;
    }>): Promise<CommandResult<DevMutationResult<DevPlannerRunRecord>>>;
}>;
...
export type DevSdk = Readonly<{
    projects: DevProjectsApi;
    planner: DevPlannerApi;
    worker: DevWorkerApi;
}>;
export declare class DevOrchestratorError extends Error {
    readonly code: "ambiguous_match" | "not_found" | "invalid_spec" | "confirmation_required" | "ui_unsupported" | "route_drift" | "tab_ownership_unavailable" | "mutation_uncertain" | "state_error";
    readonly recoverable: boolean;
    constructor(code: "ambiguous_match" | "not_found" | "invalid_spec" | "confirmation_required" | "ui_unsupported" | "route_drift" | "tab_ownership_unavailable" | "mutation_uncertain" | "state_error", message: string, recoverable?: boolean);
}
