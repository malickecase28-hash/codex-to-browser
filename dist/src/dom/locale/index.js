...
const locales = [en, de, esES, frFR, zhHK, zhTW, ja, it, vi, am, ar, bg, bs, ca, cs, da, el, es419, et, fa, fi, frCA, gu, hi, hr, hu, hy, id, isIS, ka, kk, kn, ko, lt, zhHans, ur, uk, ptBR, ptPT, pl, sk, ro, nb, ml, ru, pa, mr, tr, sw, te, tl, th, bn, ms, so, nl, sv, lv, mk, sq, sl, sr, mn, my, ta];
...
const TOOL_IDS = ["web_search", "deep_research", "create_image"];
...
const CONFIGURATION_OPTION_IDS = [
    "instant",
    "light",
    "medium",
    "high",
...
function flattenModeOption(localeList, optionId) {
    ...
    for (const locale of localeList) {
        ...
        if (modeOptions === undefined || modeOptions === null)
            continue;
...
function flattenNestedLabel(localeList, group, id) {
    const seen = new Set();
    const result = [];
    for (const locale of localeList) {
...
const builtLabels = Object.fromEntries(nonToolKeys.map(key => [key, flattenKey(locales, key)]));
const builtTools = Object.fromEntries(TOOL_IDS.map(id => [id, flattenTool(locales, id)]));
const builtModeOptions = Object.fromEntries(MODE_OPTION_IDS.map(id => [id, flattenModeOption(locales, id)]));
const builtExperienceOptions = Object.fromEntries(EXPERIENCE_OPTION_IDS.map(id => [id, flattenNestedLabel(locales, "experienceOptions", id)]));
const builtConfigurationAxes = Object.fromEntries(CONFIGURATION_AXIS_IDS.map(id => [id, flattenNestedLabel(locales, "configurationAxes", id)]));
const builtConfigurationOptions = Object.fromEntries(CONFIGURATION_OPTION_IDS.map(id => [id, flattenNestedLabel(locales, "configurationOptions", id)]));
/**
 ...
 */
export function anyLabelPattern(candidates) {
